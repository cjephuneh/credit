...
app.get("/api/admin/overview", (_req, res) => {
    ...
    const totalOutstanding = users.reduce((sum, user) => sum + user.outstandingBalance, 0);
...
app.get("/api/mobile/dashboard/:userId", (req, res) => {
    ...
    const activeLoan = loans.find((loan) => loan.userId === user.id && (loan.status === "active" || loan.status === "overdue"));
    res.json({
        user,
        activeLoan,
...
app.post("/api/mobile/loan-apply", (req, res) => {
    const { userId, product, requestedAmount, tenureMonths } = req.body;
